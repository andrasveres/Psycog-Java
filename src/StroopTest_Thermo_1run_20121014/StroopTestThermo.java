package StroopTest_Thermo_1run_20121014;
...
public class StroopTestThermo implements StroopInterface, WindowListener {
	...
	public void Ready() {
		...
		if(state.compareTo("gyakorlas")==0) {
			...
			JOptionPane.showMessageDialog(null, "Következik a Stroop feladat");
		...
		if(state.compareTo("movingball")==0) {
			...
			String ss = "Következik a Stroop feladat (THERMO)\n\nInstrukció (emlékeztető): \n" +
			...
			StroopKeypadNew st = new StroopKeypadNew(next_test, false, names, colors, this);
			st.frame.setTitle("Stroop Thermo");			
			
			return;
		}
		
		if(state.compareTo("stroop_thermo")==0) {
			
			state = "movingball_thermo";
			JOptionPane.showMessageDialog(null, "Következik a motoros feladat! (THERMO)\n\n" +
					"Instrukció (emlékeztető):\n" + 
					"Ebben a feladatban egy mozgó sárga kör közepén lévő \n" +
					"fekete pontot fogsz követni az egérrel. Minél közelebb \n" +
					"sikerül kerülnöd a középponthoz, annál kisebb a sárga \n" +
		...
		if(state.compareTo("movingball_thermo")==0) {
	...
	void ShowMovingBall() {
        ...
        File file= new File("chart_"+participant+"_motoros_"+nowFormatted+".png");
        try {
 			ChartUtilities.saveChartAsPNG(file, chart, 800, 600);
 		} catch (IOException e) {
 			// TODO Auto-generated catch block
